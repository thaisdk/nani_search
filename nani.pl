...
% to made it compatible with SWI-Prolog
...
nshelp:-
  ...
  write('   put on something      (ex. put on flashlight)'),nl,
...
hint:-
  ...
  write('you need battery to turn on the flashlight'), nl,
...
init_dynamic_facts:-
  ...
  assertz(location(wardrobe, kitchen)),
...
noun(thing, soap) --> [soap], {have(soap)}.
